...
**유스케이스**: 버스 등 흔들리는 이동수단에서 화면을 볼 수 없을 때, 이어폰으로 단어를 듣고 암기하고 싶은 니즈.

**기본 흐름**: 🔊 일본어 단어 → ⏸ 딜레이(뜻 떠올리기) → 🔊 한국어 뜻 → 다음 단어 반복

**기술 검토 결과**:

- `speechSynthesis` (Web Speech API)는 일본어/한국어 모두 지원하나, 브라우저가 백그라운드로 가거나 화면이 꺼지면 중단될 수 있음 (특히 iOS Safari)
- **권장 구현 방향**: Wake Lock API(`navigator.wakeLock.request('screen')`)로 화면 꺼짐 방지 + 별도 듣기 모드 페이지에서 Lesson 선택 후 자동 재생
- 화면을 끈 상태에서도 재생하려면 외부 TTS API + `<audio>` 태그 방식이 필요하나 오버엔지니어링 영역
