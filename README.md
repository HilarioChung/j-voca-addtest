...
- **플래시카드 열람**: 학습 알고리즘과 별개로 Lesson별 단어를 랜덤 셔플 플래시카드로 자유 탐색
- **듣기 모드**: 일본어 발음 자동 재생 (3초 간격) + Wake Lock으로 화면 꺼짐 방지. 이동 중 이어폰 학습용
- **발음 듣기**: 복습 카드 뒷면에서 Web Speech API로 일본어 발음 재생
- **GitHub 동기화**: 단어 데이터를 GitHub repo에 자동 커밋
- **PWA**: 홈 화면 설치, 오프라인에서 기존 데이터 열람/복습 가능 (단어 추가/수정은 온라인 필요)
...
**유스케이스**: 버스 등 흔들리는 이동수단에서 화면을 볼 수 없을 때, 이어폰으로 단어를 듣고 암기.

**구현**: 단어 탭 > "듣기 모드" 버튼 > 일본어 발음 자동 재생 (3초 간격) > 랜덤 셔플 순서로 전체 반복. Wake Lock API로 화면 꺼짐 방지. 한국어 뜻은 TTS로 읽지 않고 머릿속으로 떠올리는 능동적 학습 방식.

**제약**: `speechSynthesis`는 브라우저가 백그라운드로 가거나 화면이 꺼지면 중단될 수 있음 (특히 iOS Safari). 화면을 켜둔 상태에서 사용해야 함.
